...
>
> **Deliberately NOT fixed (design decisions, not safe silent fixes):** H7 chip respelling (moebius 8-packed→4-complex, native+andy→legacy op) — needs a spelling-preservation decision; H7 garbage-row fabrication defaults + param import validation parity — load-path behavior changes; G4 solve-score statement-local line/col — parser surgery; `_format_number` dedupe into named policies — touches fingerprint-producing code paths for zero behavior change; `solve_metric_min_roots` drift test — no Python counterpart table to pin against. **Docker ARM64 runtime validation: DONE** (per `deployment-checklist.md` §8 — rebuild first, test second): `lambda/sweep` (static musl ARM64) and `lambda/sweep_coeffgen` (AL2023 Docker + LAPACK) rebuilt from the changed `sweep_cli.c`, then `bash scripts/test-docker-runtime.sh` → **All Docker runtime tests PASSED** (solve/score/clip suites + 1104-function catalog degree parity against the fresh deploy binary). The tracked `sweep_coeffgen` copy carries the assume-unchanged flag and is rebuilt at deploy time; not committed.

> **DESIGN-DECISION BATCH LANDED (2026-07-02, `498120c..d51892c`, 5 commits), each verified by repro + gates:**
> - **`498120c`** — truthiness tail closed: C2 `choices or _ENUM_ARG_VALUES` → hard-fail on missing enum choices, C5 `_profileSelectorChoices` `.length` fallback → presence check (`[]` is a valid selector list), C6 `selectors.get(name) or []` → presence semantics with ValueError on malformed profiles; M3 oracle corpus counts pinned exactly (EXPECTED_TOTAL_CASES + per-kind EXPECTED_CHECKED floors — the "0 checked = pass" hole closed); 18 more test files gated (~72 total) + ALLOWED_UNGATED snapshot frozen.
> - **`8d5a300`** — H7 fabrication half: garbage chip rows (unknown names, `{}` rows) now REJECT at import instead of fabricating `unit_circle`/`rev` defaults; param import gains the same UI-chain validation as coeff (`_validateParamProgramUiChain`); const imaginary-lane `(a)+(b)*1j` fix.
> - **`dd6236b`** — G0 residue: the two frozen number policies now have ONE named home each in `program_source_core` (`canonical_number_g17` = .17g+signed-zero-fold, param/coeff v1 wire; `canonical_number_repr` = repr+int-collapse+zero-fold, root/solve) with delegating call sites and parity drift tests (`solve_score_chain` keeps a local copy for lean-bundle packaging, parity-pinned).
> - **`43d47a8`** — H7 respelling half: chip hydration stashes the original row (`savedRow`); pristine serialize emits it verbatim so load→save round-trips are byte- and fingerprint-identical; any edit respells. Harness-pinned both ways.
> - **`d51892c`** — G4 closed: solve-score parser records `statement_spans`; chain-stage compile errors are attributed to their statement by cumulative-prefix replay (first prefix reproducing the same message owns line/col). Bad quantile on line 3 reports line 3, omega inf on line 2 reports line 2.
>
> **DEPLOYED & VERIFIED (2026-07-02):** user deployed at `d51892c`; `./deploy.sh show-build` → **Overall: MATCH** (build `20260702-193159-d51892c`); §19 post-deploy INIT sweep passed (exit 0).
>
> **FINAL DEFECT + TAIL BATCH LANDED (2026-07-02, `d7dc5d2..83ba7b7`, 4 commits) — closes every remaining verified-open item:**
> - **`d7dc5d2`** — the four genuine defects: (1) coeff expr parser gains a **wire-preserving** static-zero-denominator guard (`_term` + negative-exponent reciprocal; validate-don't-fold — emitted tokens stay byte-identical for all valid programs; `t1+1/0`, `t1+1/(2-2)`, `t1+0**-1` now reject at compile instead of failing every row at runtime, matching param); (2) `classify_source_error_code` in `program_source_core` upgrades the bare `source_error` default from the message inside both `CoeffProgramSourceError` and `SolveScoreProgramSourceError` `__init__` — ~90 uncoded raise sites gain branchable codes without editing any site; explicit codes always win; the source-equivalence oracle comparison allows refinement of the legacy default only, specific legacy codes still match strictly; (3) `solve_metric_min_roots` C switch drift-pinned to a test-side table (explicit cases + `default:2` + no stale names) — the last un-pinned `solve_score.h` table (closes the Open #5 residue); (4) coeff `_compile_expr` real-arg check folds statics and judges the value (`1j*1j` ≡ `-1` accepted, folds to identical tokens+fingerprint as `-1`; dynamic/truly-complex still rejected) — param parity, closes the G-series LOW.
> - **`536a97a` + `83ba7b7`** — **pow.args normalized** to its two-complex display shape (the review's preferred Phase-5 outcome, not the fallback exception): packing never read pow's declared args (compat_signatures short-circuit every consumer — `_legacy_args`, `_max_native_transform_stack_arg_count`, andy split, stack-arg typing; verified by site audit), M3 oracles + wire fingerprints + source equivalence + native parity all green, regenerated `coeff_vocab_js.js` **byte-identical**; `COEFF_PACKED_DISPLAY_OUTLIERS` and its schema carve-out **deleted** — every coeff function now satisfies the general ui.params == runtime-args rule. **No schema exemptions remain.**
> - **`095eb8e`** — tail: C7 presence semantics for title/help coalescing; `translate_root_from_old` + 3 private helpers deleted (zero production callers — root programs never shipped a v1→v2 payload migration; deletion pinned); solve v2 translator return dict gains `program_kind` (return-dict metadata only, the `_v2_fingerprint` payload is frozen wire); dead persisted fields removed (`solve_score_has_source_text` S3 metadata + `has_source_text` summary — zero consumers, the modal renders id/name/statement_count/saved_at; persisted-body long-key source_text alias — bodies are compiled dicts and always carry `source_text`; the request-param aliases stay, the frontend sends them); param `statement_count` aligned to coeff semantics (source statements when saved from text, lowered rows otherwise) and the now-duplicate `source_statement_count` payload key dropped; js/02 nested `pipeline/detail.{param,coeff}_program.source_text` probes deleted (compiled program payloads never carry `source_text`) with harness absence pins; tautological structuralChips/programProfiles self-comparisons dropped (the disk-vs-`render_js()` comparison is the real gate); orphaned `bench_baseline.json` deleted (host-specific timings, no gate references, `--write-baseline` regenerates); `_PROFILES` memo try/finally restore; G9 non-round numeric root round-trip corpus test (repr policy: fingerprint-identical round-trips, nearby values distinct).
>
> **Full predeploy gate green at `83ba7b7`: 859 tests + frontend harness.** Deploy is one batch behind — a redeploy picks up `d7dc5d2..83ba7b7`; no native/C changes in this batch, so the Docker ARM64 gate is not required (§8 applies only when `sweep_cli.c`/binaries change).
>
> **CLOSED BY DECISION (accepted debt — recorded so nobody re-opens it silently):**
> - `.17g` (param/coeff) vs `repr` (root/solve) display split stays: both are frozen fingerprint wire, and the frozen shell oracles pin source-text equality byte-for-byte — "unifying display" would fork display from fingerprint text, which is worse than the split. The two policies are named, single-homed, and parity-pinned (`dd6236b`).
> - Parser-level empty/blank-source asymmetry between kinds stays: the G8 four-case matrix pins per-kind behavior as contract.
> - F5 registry-declared under-arity policy: explicit in code + drift-pinned; moving it into registry JSON is churn without a consumer.
> - T4 structured strict=False warning channel: RuntimeWarnings suffice for the two internal callers.
> - Harness grep-anchoring weakness + vocab mtime freshness heuristic: known, tolerated; the predeploy gate re-running everything is the backstop.
...
5. **C-executor runtime single-sourcing:** the **drift tests landed** (`67ff066`: `dispatchPt` defaults vs registry, `coeffLegacyApply` fallbacks, native packing special-cases vs `compat_signatures` — all pass), so the earlier "decorative registry" risk is now pinned. ~~Still open: `solve_score.h` metric-partition/min-roots tables have no drift test~~ → partition pinned in `80de7f1`, min_roots pinned in `d7dc5d2`; every `solve_score.h` table now has a drift test. C remains the runtime authority for omitted args (no Python-always-expands contract) — accepted.
...
Every finding section below was re-checked against `ba0f962` by re-running its repro (four parallel deep-dives; severity-critical items re-run independently). Summary — the body text of each finding describes the **as-found** state; this table was current truth at `ba0f962` and is superseded by the 2026-07-02 batch blocks above (in particular: C2/C5/C6/C7 truthiness fixed, F3 chain-diagnostic codes landed, G4 statement attribution landed, H7 both halves landed, G0 residue named+pinned, pow exemption removed):
...
>
> **Update `d7dc5d2`: CLOSED for tables** — metric partition pinned in `80de7f1`, `solve_metric_min_roots` pinned in `d7dc5d2` (parsed switch vs test-side table, explicit cases + default + no stale names). The omitted-args runtime-authority point is accepted debt.
...
> **Update `536a97a`+`83ba7b7`: DONE, preferred outcome achieved** — `pow.args` normalized to two complex args (`multiplier`, `exponent`, defaults matching the `complex_pair` signature); wire-safe proven (M3 oracles, wire fingerprints, source equivalence, native parity green; regenerated vocab byte-identical — the declared args were never read by packing, only compat_signatures are). `COEFF_PACKED_DISPLAY_OUTLIERS` schema exception deleted.
