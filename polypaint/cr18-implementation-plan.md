...
**Implementation status (2026-06-20): parser-shell slice landed.** `coeff_program_source.py` now delegates statement splitting, top-level call parsing, top-level assignment detection, source-span records, and diagnostic-code formatting to `program_source_core.py`. Strict-mode Coeff source failures now raise a `RuntimeError` subclass carrying structured `{line,column,level,code}` diagnostics. Coeff semantic lowering still lives in `coeff_program_source.py`, and the frozen old-parser oracle/corpus equivalence gate below is still not complete. This is a real shared-core step, but it is **not** the Phase-4 release gate yet.
