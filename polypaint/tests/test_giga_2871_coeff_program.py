...
def _load_generator():
    path = os.path.join(ROOT, "scripts", "gen_giga_2871_coeff_program.py")
    spec = importlib.util.spec_from_file_location("gen_giga_2871_coeff_program", path)
...
def _snapshot_row(generator, t1, t2):
    ...
    500685f). Asserts the sorted-leading == 1 regime: the monic term is the
    magnitude minimum, so the sorted polynomial stays monic degree 32."""
    import numpy as np

    roots = np.array(generator.letter_roots(), dtype=np.complex128)
    cf = np.poly(roots + complex(0.05 + 0.9 * t1, 0.05 + 0.9 * t2))
    out = cf[np.argsort(np.abs(cf))]
    assert out[0] == 1, "sort_abs regime violated: leading not the monic 1"
    return out


class TestGiga2871CoeffProgram(unittest.TestCase):
    def test_generated_document_is_fresh_portable_and_compiles(self):
        ...
        self.assertEqual(stored["name"], "giga_2871")
    ...
    def test_vm_rows_match_snapshot_formula(self):
        ...
        1e-9 relative; measured headroom ~1.2e-11 (translate-vs-np.poly
