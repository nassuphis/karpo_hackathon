...
The first implementation pass landed the behavior-preserving work that can be
gated without changing fingerprinted Coeff wire packing:

- Param grammar now reads its source vocabulary from `profiles.param.source` and
  lowers through `program_source_core.ProfileStatementLowerer`.
- Param frontend vocabulary is generated by `lambda/gen_param_vocab.py` and
  shipped as `param_vocab_js.js`.
- Coeff registry loading now materializes `effective_args` and generated vocab
  carries `andy`; Help no longer appends `andy` as a UI-only fiction.
- Coeff vector binary/unary frontend and source-parser membership now derive
  from `structural_chips.json`, exposing `real` and `imag`.
- Help is generated from the profile/vocab artifacts, has separate Starter/Help
