...
class TestDeepZoomFromRaw(unittest.TestCase):
    @patch("handler_deepzoom_export.report_status")
    @patch("handler_deepzoom_export.s3")
    @patch("handler_deepzoom_export.subprocess")
    def test_export_from_raw_requires_and_uses_sidecar(self, mock_subprocess, mock_s3, mock_report):
    ...
    @patch("handler_deepzoom_export.report_status")
    def test_export_from_raw_rejects_missing_sidecar(self, mock_report):
        import handler_deepzoom_from_raw as mod

        with self.assertRaisesRegex(RuntimeError, "requires raw_key and raw_meta_key"):
            mod.handler(_event(raw_key="", raw_meta_key=""), None)
