...
def _load_generator():
    path = os.path.join(ROOT, "scripts", "gen_giga_2873_coeff_program.py")
    spec = importlib.util.spec_from_file_location("gen_giga_2873_coeff_program", path)
...
def _snapshot_row(generator, t1, t2):
    ...
    only faithfully reproducible behavior (recreate_giga_2873.md §1.5)."""
    import numpy as np
...
class TestGiga2873CoeffProgram(unittest.TestCase):
    def test_generated_document_is_fresh_portable_and_compiles(self):
        ...
        self.assertEqual(stored["name"], "giga_2873")
        self.assertEqual(stored["chain"], [])
        self.assertEqual(len(generator.letter_roots()), 34)
    ...
    def test_vm_rows_match_snapshot_formula(self):
        """Native VM vs verbatim andy1 at four grid points. Acceptance 1e-8
        relative: measured headroom ~4.0e-11 — the residual is
        translate-vs-np.poly expansion-order dust magnified by degree-34
        binomials at the 3.9e15 coefficient scale."""
        import numpy as np
        ...
        for t1, t2 in ((0.137, 0.823), (0.75, 0.31), (0.0, 0.999), (0.5, 0.5)):
            ...
            self.assertLess(float(rel.max()), 1e-8, (t1, t2, float(rel.max())))
