...
Under wire-compatible cleanup (A): the **source** parser/serializer use the canonical complex form (`linear(2+3j, 0)`); the **compiler still emits the existing `_token` wire** — the same `args`/`args_im`/`expr_refs` bytes — for these fingerprinted functions, driven by the `compat_signature` as the declared wire layout. (The "four reals" is the legacy *source* spelling, not the wire: the wire already carries two complex args across the real/imag lanes, so `linear(2+3j,0)` and old `linear(2,3,0,0)` compile to the identical `_token`.) So this **collapses** the five hardcoded packers + three inverse cases + two JS copies into **one data-driven complex↔wire codec** — it does not remove packing (the wire is permanent). Net is still a large deletion (~150+ lines of per-`fn_index` ladders → one codec), framed as "make packing data-driven and singular," gated by **byte-identical fingerprints over the saved-program corpus**.

The codec must reproduce **each function's exact existing lane usage** — confirm per function, since not every legacy path necessarily populates `args_im` (`linear`/affine and round-style paths do, at `:1064/1170/1218`; verify `exp`/`pow`). The per-`compat_signature` wire-layout declaration plus the byte-identical-fingerprint gate are exactly what enforce this — a per-function implementation check, not a design gap.
