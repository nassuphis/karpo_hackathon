...
class TestRenderLoresPreviewHandler(unittest.TestCase):
    ...
    @patch("handler_render_lores_preview.subprocess.run")
    @patch("handler_render_lores_preview.s3")
    def test_view_render_composes_generate_and_rasters_the_projection(self, mock_s3, mock_run, mock_render):
        # A VIEW is an associated artifact of the selected color render: the
        # mode composes the artifact generate core (cached dump), subsamples
        # the STORED step_scores at the lattice, and runs view_raster with the
        # artifact's own viewport — persisting under renders/{job}/views/.
        from handler_render_lores_preview import (
            TMP_SPLAT_ROOTS, TMP_VIEW_RAW, TMP_XFORMED_ROOTS, handler,
        )
        ...
        def subprocess_fake(cmd, **kwargs):
            if cmd[0].endswith("view_raster"):
                seen["view_cmd"] = list(cmd)
                scores_arg = next(a for a in cmd if a.startswith("--scores="))
                with open(scores_arg.split("=", 1)[1], "rb") as fh:
                    seen["view_scores"] = fh.read()
                with open(TMP_VIEW_RAW, "wb") as fh:
                    fh.write(bytes([7]) + bytes(64 * 64 - 1))
                return MagicMock(returncode=0, stdout=json.dumps(
                    {"pix": 64, "channels": 1, "plotted": 9, "clipped": 2,
                     "deduped": 5, "rmax": 2.8284271247461903}), stderr="")
            with open(TMP_XFORMED_ROOTS, "wb") as fh:
                fh.write(b"T" * (16 * 1 * 2 * 2))
            return MagicMock(returncode=0, stdout=json.dumps({"roots_plotted": 16, "roots_clipped": 0}), stderr="")

        def render_fake(**kwargs):
            seen.setdefault("renders", []).append(dict(kwargs))
            with open(kwargs["out_path"], "wb") as fh:
                fh.write(PNG_1X1)
            return {"file_size": len(PNG_1X1), "preview_file_size": 0}
        ...
        self.assertEqual(view["palette"], "viridis")
        self.assertEqual(view["plotted"], 9)
        self.assertIn(f"/renders/j/views/{view['view_id']}/image.png", view["image_url"])
        # the tool ran on the ARTIFACT's provenance: its viewport, the
        ...
        self.assertIn("--pix=64", cmd)
        self.assertIn("--channels=1", cmd)
        self.assertIn("--roots_format=u16", cmd)
        self.assertIn("--min_re=-2.0", cmd)
        self.assertIn("--max_im=2.0", cmd)
        self.assertIn(f"--roots={TMP_SPLAT_ROOTS}", cmd)
        self.assertTrue(body["provenance"]["roots_reused_locally"])
        self.assertEqual(
            list(seen["view_scores"]),
            [0, 1, 2, 3, 7, 6, 5, 4, 8, 9, 10, 11, 15, 14, 13, 12])
        # the VIEW image renders with the artifact palette; unclaimed pixels
        # stay background (zero_background defaults True — unlike the palette
        # PNG, where every lattice cell is real data)
        view_render = seen["renders"][-1]
        self.assertEqual(view_render["palette"], "viridis")
        self.assertEqual(view_render["background_color"], "101010")
        self.assertEqual(view_render["pix"], 64)
        self.assertNotIn("zero_background", view_render)
        # persisted pair: immutable image + no-cache meta
        by_key = {c.kwargs["Key"]: c.kwargs for c in mock_s3.put_object.call_args_list
                  if "/views/" in c.kwargs["Key"]}
        self.assertEqual(set(by_key), {f"renders/j/views/{view['view_id']}/image.png",
                                       f"renders/j/views/{view['view_id']}/meta.json"})
        img = by_key[f"renders/j/views/{view['view_id']}/image.png"]
        self.assertEqual(img["ContentType"], "image/png")
        self.assertIn("immutable", img["CacheControl"])
        meta_row = json.loads(by_key[f"renders/j/views/{view['view_id']}/meta.json"]["Body"])
        self.assertEqual(meta_row["source_artifact_id"], "color_run_abc")
        self.assertEqual(meta_row["image_key"], f"renders/j/views/{view['view_id']}/image.png")
        calls = mock_report.call_args_list
        self.assertEqual(calls[0].args, ("j", "view_render_7", "running"))
        self.assertEqual(calls[-1].args, ("j", "view_render_7", "done"))
        self.assertEqual(calls[-1].kwargs["result_data"]["view"]["view_id"], view["view_id"])

    @patch("handler_render_lores_preview.s3")
    def test_view_render_rejects_bad_projection_and_reports(self, mock_s3):
        from handler_render_lores_preview import handler
        ...
        with patch("handler_render_lores_preview.report_status") as mock_report:
            resp = handler(_event(
                view_render={"source": {"artifact_id": "color_run_abc", "n": 4},
                             "params": {"projection": "top", "vertical": "t2", "pix": 64}},
                sculpture_task_id="view_render_8",
            ), None)
        self.assertEqual(resp["statusCode"], 500)
        self.assertIn("projection", json.loads(resp["body"])["detail"])
        calls = mock_report.call_args_list
        self.assertEqual(calls[0].args, ("j", "view_render_8", "running"))
        self.assertEqual(calls[-1].args, ("j", "view_render_8", "error"))
