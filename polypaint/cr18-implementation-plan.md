...
# --- Coeff profile adds (Param above is a subset of this one broad grammar) ---
indexed_assign := indexable "[" expr "]" "=" expr            # Coeff: poly[i] = expr  (Param has no indexed lvalue)
indexable     := <symbol: write access + index context>      # Coeff: poly
...
# LAYOUT CONTRACT: legacy/native calls source-first [name,src,tgt,…]; named chips target-first [tgt,src,…] (coeff_program_chain.py:994)
```
...
- **Two steps — flip, then delete (the equivalence gate needs *both* parsers live):** Step 1 flips production to the shared-core coeff profile but **keeps the old coeff parser as a test-only/legacy adapter** (off the production path) so CI can keep comparing old-vs-shared-core. Step 2 — a **follow-up cleanup** — deletes the old parser **only after** equivalence is green and the v2 release gate has passed. Production runs exactly one parser (shared-core) throughout, so there's no production divergence; the old parser survives solely as the test oracle until cleanup. (Deleting it in the flip change would leave the equivalence gate nothing to compare against.)
- **Corpus:** every saved coeff program (the `v1/` store) **plus** a hand-written landmine corpus covering the §3.1 hard cases — macro expansion, typed-stack expressions, every selector slot, indexed reads/writes, `poly_len`, `tosN`, both call layouts, and the strict/non-strict error-recovery paths.
- **Equivalence asserted per corpus item:** old coeff parser vs shared-core coeff profile must agree on (a) the **compiled chain**, (b) the **fingerprint**, (c) **diagnostics** — compared on `{line, column, level, code}`, **not raw message text** (brittle). Today's diagnostics are only `{level, message, line, column}` (`coeff_program_source.py:137-140`), so the shared core **adds a stable `code` field** (a diagnostic-code enum it owns), for both the strict and non-strict paths; if codes aren't added, the gate falls back to `{line, column, level, broad-category}` but never raw text. And (d) **canonical source regeneration**, which needs a serializer that **does not exist today**: add backend **`coeff_source_text_from_chain(chain)`** — canonical, re-parseable, mirroring the param `param_source_text_from_chain` (§3.1) — distinct from `display_coeff_program_chain` (`coeff_program_chain.py:877`, human-readable) and the UI-local `_coeffProgramSourceFromRows` (`js/08-chip-editors.js:310`, which is limited to display-only). The round-trip `parse(coeff_source_text_from_chain(c))` must yield an equal `execution_spec`/fingerprint.
