/*
 * coeffs_bilevel_raster: project one coeff stripe into per-tile bitset files.
 *
 * Reads a coeffs .bin file (f32 complex coefficient pairs), projects each root to pixel
 ...
 *
 * No RGB. No color. No matching. Just occupancy bits. Coefficients, not roots.
...
int main(int argc, char **argv) {
    if (argc < 3) {
        fprintf(stderr, "Usage: coeffs_bilevel_raster coeffs.bin outPrefix [options]\n");
        return 1;
    }
