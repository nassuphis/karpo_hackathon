...
- **[D-Node Paths](docs/d-node-paths.md)** — D-List tab, D-node path animation across all 5 entry points, fast mode worker integration, save/load
- **[Testing](docs/test-results.md)** — 430 Playwright tests covering solver, root tracking, paths, shapes, colors, stats, save/load, morph, jiggle, fast mode, off-canvas render, multi-format image export, bitmap/animation color decoupling, derivative bitmap coloring, root-matching strategies (Hungarian algorithm), D-node paths, animation entry points, and JS vs WASM benchmarks
...
**Left panel** has four tabs: **Coefficients** (complex-plane visualization with drag interaction), **List** (tabular view with per-coefficient columns for position, speed, radius, curve length, and curve index; a **Transform** dropdown with 22 bulk operations; and **Param1/Param2** sliders for transform parameters), **D-List** (identical tabular view for morph target D-nodes — assign paths, speeds, and transforms to D-nodes independently from C-coefficients), and **Morph** (morph panel with interactive C/D visualization, blending rate control, and interpolation markers). The tab bar also contains a **scrub slider** (drag to manually move C and D coefficients along their paths, 0–5 seconds), ▶ **Play** / ⏸ **Pause**, and **Home** (return all animated C and D nodes to start positions).
...
│   ├── d-node-paths.md   # D-List tab, D-node path animation, fast mode workers
...
├── tests/                # Playwright Python tests (430 tests, 20 files)
