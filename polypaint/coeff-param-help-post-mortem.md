...
**Pre-M3 oracle/corpus gate:** no longer deferred in current worktree. `lambda/coeff_program_chain_legacy.py` and `lambda/param_program_source_legacy.py` are frozen, first-line-marked test oracles; `lambda/coeff_program_source_legacy.py` now compiles through the frozen chain oracle. `tests/fixtures/program-m3-oracle/` contains calc-shaped source/chain fixtures, and `tests/test_program_m3_oracles.py` compares current vs frozen parsed chains, execution specs, fingerprints, token/scalar counts, and Param chain→source regeneration. The test is wired into `scripts/predeploy_check.sh` and asserted by `tests/test_deploy_packaging.py`. This is now the hard prerequisite gate before any M3 wire-packer rewrite. **Independently re-verified at `adfa918`:** the oracle modules import only stdlib + the *frozen* `program_source_core_legacy` (which itself has **zero** imports of the live param/coeff/core modules) — so they are true frozen references, not self-referential copies of the live packers. By construction the gate fails the build the moment a future M3 packer change moves any `fingerprint`/`execution_spec`; it is green now only because the packers are unchanged (4 passed, 6 subtests).
...
**Forward (not a defect):** the M3 wire-packer rewrite itself has not been attempted — but it is now *unblocked*. The frozen oracle will catch any wire divergence, so M3 can proceed safely whenever it is taken up.
