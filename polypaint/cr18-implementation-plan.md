...
**Objective.** A JSON registry for the **program-chip metadata** (the `_CHIP_COMPILERS` stack-VM vocabulary) plus a **profile symbol table** for param/coeff/root/solve-score. The chip registry drives generated frontend/profile metadata and drift gates for opcode/sub-op/selector/arg-shape agreement; it does **not** replace the hand-maintained compiler dispatch maps. The semantic compiler handlers stay as named Python functions. The symbol table makes identifiers (`t1`, `p1`, `cf`, `poly`, `poly_len`, metric slots, roots) profile configuration, not parser magic. No runtime VM change: Phase 1 feeds Python parser/validation/frontend metadata, while C keeps its existing constants and receives a drift test.
...
Per entry (modelled on the as-built metadata in `lambda/structural_chips.json`):
```jsonc
{ "name": "affine", "aliases": ["linear"],
  ...
  "selector_slots": { "src": "src", "tgt": "tgt" } }
```
Arg `type ∈ {int, real, complex, selector, enum, registry_ref, scalar_expr, complex_expr, length_expr}`. Internal/synth-only chips are represented by their names and args but are not exposed as authoring commands. The registry intentionally does **not** carry stack-effect or `lower_to` handler data in the shipped implementation; those remain compiler/runtime semantics pinned by source tests and native parity tests, not by JSON metadata. The **full inventory** is the per-chip table the audit produced: 30 `COEFF_OP_*` entries spanning `push_const/push/emit/dup/swap/pop/flush/blend/legacy/poke_poly/poke_tos/vector_binary(+5 fns)/vector_unary(+16 fns)/vector_roll/argsort/littlewood/linspace/range/set/affine/typed_push_scalar/typed_push_vector/typed_binary/typed_unary/typed_get_scalar/typed_set_poly/typed_poke_poly/typed_fill/native_transform(+stack_args)/typed_blend/macro` — plus the Tier-D registry-name fallthrough (`coeff_program_chain.py:1642`). Capture the arg-order quirk: legacy/native rows are **source-first** `[name,src,tgt,…]`, named chips are **target-first** `[tgt,src,…]` (`coeff_program_chain.py:1570,1636`; the named "LAYOUT CONTRACT" comment lives in `js/08-chip-editors.js:998`).
...
Today `build_vocab()` emits `window._coeffRegistryVocab = {names, fnIndexByName, aliasToCanonical, sourceAliasByName, chipNameByRegistryName, supportsAndy, ctCatalog, categoryMeta, programParamDefs, structuralChips, programProfiles}` to `coeff_vocab_js.js`. The shipped generator:
- **Python:** does not generate `_CHIP_COMPILERS`; the compiler dispatch remains hand-maintained and drift-checked against `structural_chips.json`.
- **C:** does not consume the structural-chip JSON; C constants/opcodes remain hand-reviewed and are drift-checked against the JSON/Python mirrors.
- **JS:** consumes `structuralChips` and `programProfiles` from `coeff_vocab_js.js`; `program_profiles_js.js` is also generated for the profile-only mirror.
...
The existing test pins `COEFF_OP_*` / `COEFF_EXPR_*` / `COEFF_SEL_*` enums + the 28-transform fn_index + aliases + generated-JS byte-equality (`:45,:113,:152,:190`). The shipped drift gate additionally pins structural-chip `name → opcode → args → selector_slots` across registry/Python/C/JS and verifies the structural inventory covers the Python compiler names. It does **not** pin `stack_effect`, because that field is not part of the shipped structural schema. Param opcode/expression/profile drift is covered by `test_param_program_drift.py`.
...
**File map:** new `lambda/structural_chips.json`, new `lambda/program_profiles.json`, generated `lambda/program_profiles.py` + root-level `program_profiles_js.js` mirror + C drift-report/header, `param_legacy_registry.json` drift coverage; `gen_coeff_vocab.py` emits structural/profile metadata into `coeff_vocab_js.js`; `coeff_program_chain.py` / `param_program_chain.py` keep hand-maintained compiler dispatch while identifier/profile validation and drift gates use the registries; generated C drift header/report; `js/07-transform-catalogs.js` consumes the generated catalog; `tests/test_coeff_program_drift.py` + new `test_param_program_drift.py` + new `test_program_profiles_drift.py`. **Risk:** low-medium — bigger than a vocab-gen (the chips carry compiler semantics, §CR18 Phase 1), with expanded validation/frontend blast radius but **no runtime behavior change**; the drift gate de-risks divergence.
...
`param_program_source.py` and `program_source_core.py` are compute-path → the same 5 bundles as `coeff_program_source.py`: coeffgen, storage, compute_preview, param_debug, compute_plan (`deploy.sh:964,985,1034,1058,1328`). `program_profiles.py/json` must ride with every bundle that imports either source parser. Once Coeff is migrated to `program_source_core.py`, the existing `coeff_program_source.py` entrypoints remain the public module names but delegate to the core, so downstream imports and payload contracts do not churn. A missing `cp` is a runtime ImportError caught by `test_deploy_packaging.py`'s AST walk (static imports only). **No root source module** (3.2). **Gates:** `test_param_program_*` (native + storage + source round-trip), shared parser-core unit tests, profile-symbol diagnostics tests, the AST packaging gate, api_manifest regen for the new route, and a Coeff shared-core equivalence suite as a **Phase 4 release gate** — must pass before v2 ships, not before Phase 4 starts (`source → old coeff parser shell → chain/fingerprint` equals `source → shared core coeff profile → chain/fingerprint` over the checked-in hand-written landmine corpus today; saved-program snapshot/export coverage remains a release-gate hardening follow-up; full breakdown §3.6). **Risk:** low-medium for the Param increment; medium for the mandatory Coeff migration because it touches a live, fingerprint-critical parser.
...
**Implementation status (2026-06-20): production flip complete; oracle retained.** `coeff_program_source.py` now delegates statement splitting, top-level call parsing, top-level assignment detection, source-span records, diagnostic-code formatting, selector/symbol vocabulary lookup, the top-level statement-dispatch grammar (`assignment` vs `call` vs `bare`), and the parse/lower/diagnostic orchestration to `program_source_core.py` + `program_profiles.json`. The production Coeff path is a profile-backed `ProfileStatementLowerer` hook class; `coeff_program_source_legacy.py` keeps the frozen old Coeff parser shell as a test-only oracle, never as a production import. The oracle intentionally imports production semantic lowerers, so it proves parser-shell equivalence, not a fully frozen pre-refactor lowerer. Strict-mode Coeff source failures raise a `RuntimeError` subclass carrying structured `{line,column,level,code}` diagnostics. `tests/fixtures/coeff-program-corpus/` plus `test_coeff_source_equivalence.py` compare the legacy shell vs shared-core/profile-hook output over a checked-in hand-written corpus, including compiled `execution_spec` + fingerprint equality. A saved-program snapshot/export script is still desirable to broaden the release gate beyond the hand-written corpus. The canonical source serializer decompiles source-generated internal `_typed_*` chains (including dynamic indexing, `tos[...]`, typed set/poke/fill, and native stack-arg transforms such as `exp_affine`) back to editable text and the oracle requires regenerated source to preserve `execution_spec` + fingerprint. Coeff chip/value-specific lowering remains in the Coeff profile hook module by design; the parser/diagnostic/orchestration engine is shared. The old oracle is deleted only after the v2 release gate passes.
...
- **Corpus:** the shipped gate currently uses a hand-written landmine corpus covering the §3.1 hard cases — macro expansion, typed-stack expressions, selector slots, indexed reads/writes, `poly_len`, `tosN`, both call layouts, and strict/non-strict error recovery. Follow-up coverage should add every saved coeff program from a **checked-in fixture snapshot** of the existing **unversioned** store (`polypaint/coeff-programs/<id>.json` — `COEFF_PROGRAMS_PREFIX`, `handler_storage.py:75`; *not* a `v1/` prefix, with v2 to live under `polypaint/coeff-programs/v2/<id>.json`). CI must never depend on live S3: add an export script (e.g. `scripts/export_coeff_program_corpus.py`) that snapshots S3 programs into `tests/fixtures/coeff-program-corpus/saved/<id>.json`, and an index `tests/fixtures/coeff-program-corpus/corpus.json`; tests read only those checked-in fixtures.
...
**Implementation status (2026-06-20):** Phase 4 storage/native-wire groundwork has landed. Landed: `program_v2_translate.py`, v2 fingerprints (`sha256:` over `{program_kind,spec_version,payload}`), per-kind migration routes `/migrate-{coeff,param,solve-score}-program`, dry-run previews, non-overwriting writes under `polypaint/{kind}-programs/v2/<id>.json`, explicit `409` conflict and `422` missing-macro responses, backend `coeff_source_text_from_chain(chain)`, deploy/API manifest sync, migration-route tests, generated `merged_opcodes.{py,h}` plus JS mirror, native v2 decode for the executable Param/Coeff subset with `native_transform.registry` validation, solve-score `v2;` CLI sentinel handling, solve-score internal op ids in the merged 64-95 range, solve-score/root `translate_from_old` token output, `root_legacy_registry.json`, root numeric dispatch, storage packaging for the root registry, and per-modal Legacy migration actions (`Migrate v2`) for coeff/param/solve-score saved programs. Existing v1 compute/read paths remain accepted; the §3.6 Coeff shared-core release gate has landed and remains a v2-ship gate.
...
- **root** → a sequence of `native_transform[root_registry, fn_index]`. Phase 4 added `root_legacy_registry.json` assigning stable `fn_index` values to the 9 root transforms (`rotate_roots`/`pull_unit_circle`/`roots_toline`/`line_to_unit_circle`/`invert_roots`/`add_complex`/`mul_complex`/`moebius`/`pull_towards_center`), pinned by `tests/test_root_transform_registry.py`; `apply_root_xforms` now resolves old name arrays to `fn_index` at parse time and dispatches numerically at runtime. Root has its **own value model + execution context** (CR18 §7.0): root transforms run **per-sample at raster time over a `float` root array** (`root_xforms.h`), *not* over the coeff VM's `double` complex-vector registers. The root profile declares: input/output register = the `roots` float vector (in place); a **precision policy** (float vs the VM's double — pin which, since it's observable); the raster-stage invocation context; length-preserving; and the NaN-poison convention (`rt_mark_undefined`). **Unknown names → traced no-op** in translation (drop + diagnostic, never hard-fail or silently vanish).
...
The Legacy UI is implemented as a per-kind `Migrate v2` action in the existing saved-program modals, not a separate root/global popup. It loads the selected old saved program and runs `translate_from_old`, **per-kind for the saved-program kinds only**: **coeff/param → migrated text form** (Phase 3 param parser; coeff already has one); **solve-score → chain + `program_spec` preview** (no text source). **Root is not a saved-program kind and is not listed in the tab/action set** — its `root_transforms` are embedded in render/palette artifacts and migrate **in place**; `translate_from_old` still handles the root case (§4.2), but only for that embedded path, not a tab entry. Save the v2 copy under the versioned id. **Decided: root is NOT promoted to a saved-program kind** — its `root_transforms` live embedded in render/palette artifacts and migrate **in place** under the v2 palette identity (no new `polypaint/root-programs/` prefix, no root migrate route, no root entry in the Legacy UI). So the Legacy action lists/migrates only coeff/param/solve-score. UI plugs into `js/03-program-modals.js` and `js/06-popup-init.js` (the per-kind fetch/list/save/delete call sites).
...
**Implementation status (2026-06-20): landed.** Compute plan, compute preview, and coeffgen translate nonempty `param_transforms` / `coeff_transforms` into compiled Param/Coeff Program payloads before building native CLI specs, including the old Program-mode `legacy_transforms` / `legacy_coeff_transforms` fast-path cases. The native specs carry inert empty transform arrays plus `param_program` / `coeff_program`; original rows survive only as `*_transforms_display` metadata for logs, preview-context messages, and UI display. Native `parsePtChain` / `parseCtChain` are deleted, and any nonempty `param_transforms` / `coeff_transforms` that reaches `sweep_cli.c` is now a hard native-runtime error. The Param/Coeff program editors are text-first; chip strips are read-only visualization and old chain-only saved programs are synthesized into source text on load/copy. Docker/runtime regression fixtures now use compiled program payloads or assert native-chain rejection. The visible Chain/Program selector is removed; `pipeline_mode` now normalizes stale `chain`/`legacy` clients to Program at the Lambda boundary. ASL no longer forwards transform arrays or `pipeline_mode`; Step Functions passes source/program fields only.
...
- **Retiring the pre-VM by-name dispatch required boundary translation first; that gate is now closed.** `pipeline_mode_from_params` (`pipeline_programs.py`) normalizes all supported/stale modes to Program, while `param_transforms_to_program_chain` / `coeff_transforms_to_program_chain` translate old Chain rows at the Lambda boundary. `handler_compute_plan.py`, `handler_compute_preview.py`, and `handler_coeffgen.py` build native payloads with empty transform arrays plus `param_program` / `coeff_program`. Native `parsePtChain` / `parseCtChain` are deleted from `sweep_cli.c`; nonempty transform arrays fail fast through `rejectLegacyTransformChain`. `dispatchPt` / `dispatchCt` and their math helpers remain because VM legacy/native-transform tokens and benchmarks still use those helpers internally; what is retired is the JSON-chain parser boundary, not every legacy math implementation. ASL now forwards only source/program payload fields to worker Lambdas; transform arrays remain accepted only at the request/Lambda compatibility boundary and for display metadata on old saved artifacts.
