...
- **Retiring the pre-VM by-name dispatch required boundary translation first; that gate is now closed.** `pipeline_mode_from_params` (`pipeline_programs.py`) remains a classifier, while `param_transforms_to_program_chain` / `coeff_transforms_to_program_chain` translate Chain-mode rows at the Lambda boundary. `handler_compute_plan.py`, `handler_compute_preview.py`, and `handler_coeffgen.py` build native payloads with empty transform arrays plus `param_program` / `coeff_program`. Native `parsePtChain` / `parseCtChain` are deleted from `sweep_cli.c`; nonempty transform arrays fail fast through `rejectLegacyTransformChain`. `dispatchPt` / `dispatchCt` and their math helpers remain because VM legacy/native-transform tokens and benchmarks still use those helpers internally; what is retired is the JSON-chain parser boundary, not every legacy math implementation. **Two layers still remain separate:** ASL still forwards `param_transforms`/`coeff_transforms` through state I/O inertly. Final ASL cleanup is a later Chain-mode-removal step and must land with handler plan-field removal.
