...
- **Pipeline shape (§3.3 understates it).** "One VM" is one *codebase* invoked as staged configs across **four separate executables** with `.bin` files as the inter-stage wire — not one program (§7.5). And **root transforms are a general raster-stage profile**, not a solve-score sub-stage: they run per sample on a `float` root buffer in *both* solve-score paths (`roots2pix_mt.c:202`) and non-solve-score rasters (`bilevel_section_raster.c:211`). Model them as their own raster-stage config, not subordinate to solve-score.
