...
**Implementation status (2026-06-20):** the Param-source increment is implemented and gate-passing. Landed: `program_source_core.py` (shared statement/call/assignment/profile utilities), `param_program_source.py` (Param source parser + canonical `param_source_text_from_chain`), `/compile-param-program-source`, saved-program source precedence, compute-plan/preview/degree-probe/param-debug source plumbing, ASL forwarding, deploy packaging, API manifest regeneration, and regression tests. The compute-path helper treats **blank source text as absent** because orchestrators inject blank fields for ASL JSONPath stability; save/fetch still use the stricter saved-payload source rule. Not landed in this increment: the Param text-editor UI tab and the Coeff shared-core migration. Coeff remains the Phase-4 release gate described in §3.6 and must be done with the frozen old-parser oracle/equivalence corpus, not as an opportunistic production rewrite.
...
- **Equivalence asserted per corpus item:** old coeff parser vs shared-core coeff profile must agree on **every field of the compile output** — `chain` (parsed), `source_chain`, `expanded_chain` (macro-expanded), `tokens`, `scalar_exprs`, `execution_spec`, `legacy_coeff_transforms`, and the `fingerprint` (the `compile_coeff_program_chain` keys, `coeff_program_chain.py:2092-2106`) — plus diagnostics and canonical source regeneration:
  - **Diagnostics** compared on `{line, column, level, code}`, **never raw message text** (brittle). Two gaps to close first: non-strict diagnostics are only `{level, message, line, column}` (`coeff_program_source.py:137-140`) — no `code`; and **strict mode collapses to a plain `RuntimeError`** (`coeff_program_source.py:870,880`) that loses line/column. So the shared core must (i) **add a stable `code` field** (a diagnostic-code enum it owns) and (ii) **raise a structured exception** — generalize `CoeffProgramSourceError` to carry `{line,column,level,code}` and stop collapsing to `RuntimeError`. If codes aren't added, strict-mode equivalence is limited to **"raises the same broad category"** (non-strict to `{line,column,level,category}`) — never raw text.
  - **Canonical source regeneration** needs a serializer that **does not exist today**: add backend **`coeff_source_text_from_chain(chain)`** — canonical, re-parseable, mirroring param's `param_source_text_from_chain` (§3.1) — distinct from `display_coeff_program_chain` (`coeff_program_chain.py:877`, human-readable) and the UI-local `_coeffProgramSourceFromRows` (`js/08-chip-editors.js:310`, display-only). The round-trip `parse(coeff_source_text_from_chain(c))` must yield an equal `execution_spec`/`fingerprint`.
...
**Implementation status (2026-06-20):** Phase 4 has started with the storage-level migration slice. Landed: `program_v2_translate.py`, v2 fingerprints (`sha256:` over `{program_kind,spec_version,payload}`), per-kind migration routes `/migrate-{coeff,param,solve-score}-program`, dry-run previews, non-overwriting writes under `polypaint/{kind}-programs/v2/<id>.json`, explicit `409` conflict and `422` missing-macro responses, backend `coeff_source_text_from_chain(chain)`, deploy/API manifest sync, and migration-route tests. This slice **does not** switch compute execution to the v2 native wire, does not renumber native opcodes, does not add the Legacy tab UI, and does not satisfy the §3.6 Coeff shared-core release gate. Existing v1 compute/read paths remain unchanged.
...
- **Retiring the pre-VM by-name dispatch requires removing or translating Chain mode first.** `lookupCoeffTransform` (`sweep_cli.c:3246`), `parseCtChain` (`:5641`), `parsePtChain` (`:5576`) are the **live Chain-mode** parsers — `pipeline_mode_from_params` (`pipeline_programs.py:35`) still returns `chain` vs `program`, and `coeff_transforms`/`param_transforms` arrays still flow to native (`parsePtChain` called at `sweep_cli.c:7740,7799`). So Phase 5 cannot just delete them. **Decided: translate Chain-mode requests to Program v2 at the Lambda boundary** (keep the UI's Chain/Program mode *during* migration), then **delete native `parseCtChain`/`parsePtChain` only after a test proves no Chain arrays reach native** (assert no `coeff_transforms`/`param_transforms` in any dispatched native payload). Removing Chain mode from the UI is a later, separate step. This is a gating prerequisite, not a cleanup. **Boundary file map:** `pipeline_mode_from_params:35` is a pure **classifier** (returns `"chain"`/`"program"` only) — it does **not** transform. Add a **separate resolver** in `pipeline_programs.py`, `resolve_chain_to_program_v2(params)` (per-kind: `coeff_transforms`→`coeff_program`, `param_transforms`→`param_program`, compiled + v2-translated), mirroring the existing `coeff_source_text_for_run`/`parse_coeff_source_for_run` helper shape. The three compute handlers that emit the transform arrays to native — `handler_compute_plan.py` (`coeff_transforms`/`param_transforms`, `:139-146`), `handler_compute_preview.py`, `handler_coeffgen.py` — translate there *before* the native payload is built. **Program mode reaches the same chain parsers too — not just Chain mode:** when a compiled program carries `legacy_transforms`/`legacy_coeff_transforms`, all three handlers fall back to setting `param_transforms`/`coeff_transforms` from them (`handler_compute_plan.py:173,205`, `handler_compute_preview.py:253,269`, `handler_coeffgen.py:75,590`), so those arrays still reach `parsePtChain`/`parseCtChain` (`sweep_cli.c:7740,7799,7808,8065,8073,8510,9144`) in **Program** mode. Phase 5 must translate/disable these **6 fast-path sites** too; the gating test below covers both Chain-mode and Program-mode legacy arrays. **Two layers — don't conflate them:** the ASL forwards `param_transforms`/`coeff_transforms` through *state I/O* at 8 sites (`compute_workflow.asl.json.template:104,106,210,277,393,445,551,553`); the **gate is on the native CLI argv**, not the ASL state. During migration the arrays keep riding ASL state **inertly** (no ASL change *this* phase — they're simply unused once the handler builds the native command from the translated `coeff_program`/`param_program` slots). **Gating test:** assert no `coeff_transforms`/`param_transforms` in any dispatched *native CLI payload* (extend the payload-contract tests); the native-parser deletion is blocked on it passing. **Final ASL cleanup** — deleting the `.$` forwards — is a **later, separate step** tied to retiring Chain mode from the UI, and must land **simultaneously** with the handlers ceasing to emit those plan fields (a Step Functions `.$` reference to a now-missing path is a runtime error).
...
If appetite is limited: **0 + 1 + the Param half of 3** deliver versioning insurance, drift elimination, and text-first **param** (root/solve-score stay chips) **without** the v2 fingerprint migration or the native merge — a coherent stopping point that leaves chips editable and fingerprints untouched. But the full shared-language goal is not complete until Coeff also delegates to `program_source_core.py`; that equivalence gate (§3.6) blocks **v2 ship**, not Phase 4 start.
