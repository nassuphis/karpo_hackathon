...
At 10K resolution, the bitmap `<canvas>` element uses ~400MB of GPU memory (10,000 x 10,000 x 4 bytes). After running fast mode for an extended period, Chrome's GPU memory manager reclaims the canvas context — the canvas goes white and shows a stylized dead face icon (Chrome's canvas context loss indicator). This is not a bug in the application code; it's Chrome enforcing GPU memory limits.
...
1. **`bitmapPersistentBuffer`** (ImageData) — a `Uint8ClampedArray` of size `W x H x 4` bytes. This is **CPU-only memory**. No GPU involvement. It exists so we can avoid calling `getImageData()` every pass (which was the original 14x speedup from the persistent buffer optimization).
...
compositeWorkerPixels() writes to:
    +-- bitmapPersistentBuffer (ImageData, computeRes x computeRes, CPU-only)
    +-- bitmapDisplayBuffer (ImageData, displayRes x displayRes, CPU-only)
           |
       putImageData -> <canvas> (displayRes x displayRes, ~16MB GPU)
           |
...
**Split activates only when computeRes > DISPLAY_CAP (2000px).** At 1000px or 2000px, `bitmapDisplayBuffer` is null and behavior is identical to the non-split path — zero overhead.

### Pixel index contract

Workers always compute pixel indices in **compute-space**. The `paintIdx` values from workers are indices into the full `computeRes x computeRes` grid. The composite function:
