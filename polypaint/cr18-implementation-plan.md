...
**Implementation status (2026-06-20): parser-shell + oracle slice landed.** `coeff_program_source.py` now delegates statement splitting, top-level call parsing, top-level assignment detection, source-span records, and diagnostic-code formatting to `program_source_core.py`. Strict-mode Coeff source failures now raise a `RuntimeError` subclass carrying structured `{line,column,level,code}` diagnostics. `coeff_program_source_legacy.py` is a test-only frozen parser-shell oracle, and `tests/fixtures/coeff-program-corpus/` plus `test_coeff_source_equivalence.py` compare legacy-shell vs shared-core-shell output over a checked-in corpus. Coeff semantic lowering still lives in `coeff_program_source.py`, and the canonical source serializer still cannot invert arbitrary internal `_typed_*` chains. This is a real shared-core step, but it is **not** the full Phase-4 release gate yet.
