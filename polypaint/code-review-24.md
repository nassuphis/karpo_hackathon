...
> **STATUS 2 (2026-07-05, later):** P3 (ge/gt/le/lt/eq/rem + select sugar), scan's five-arg prev2 form with poly-reading seeds, P7 as `ipow` (repeated squaring, exponent int in [-64,64]), P9 (scalars ride the stack through pop — landed implicitly with the scalar-top change, pinned), and N7 (seeded property-fuzz gate) are shipped; port corpus 40/99. An adversarial review (three parallel reviewers + first-hand repro verification) then hardened the wave: ipow reciprocal via c_div_full, `-x**n` rejected as ambiguous (this grammar binds unary minus tighter than ** — silent sign-flip trap), tos-poke rerouting scoped/case-fixed/extended to tosN and poke_poly(), select/window/step reject stack-effectful args (multi-splice corruption), scan zero-denominator clamps, int-cast UB removed, locals reserve i/j and the polyN/cfN name families, and the parity gate lost its row-relative escape hatch, gained per-slot mixed tolerance and a decorrelated four-component chaos probe (same-sign perturbation cancelled exactly in Re(p1)−Im(p2) — poly_67's sensitive combination). All 40 ports re-verified under the hardened gate.
