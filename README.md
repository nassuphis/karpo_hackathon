...
| **Bitmap** tab | Accumulates root positions as single-pixel stamps on a canvas. Toolbar buttons (left to right): **init** snapshots animation state, clears bitmap, and resets elapsed to 0. **save** downloads a hi-res PNG. **clear** resets the canvas pixels (elapsed unchanged). **Resolution** dropdown (1000/2000/5000/10000/15000 px). **start**/**stop** toggles continuous fast mode (parallel Web Workers). Stopping preserves all state; resuming continues where it left off. **ROOT/COEF** toggles between plotting root or coefficient positions. **Steps** dropdown (10/100/1K/5K/10K/50K/100K/1M) sets solver steps per pass. **jiggle** opens a popup to perturb coefficient paths at regular intervals — 10 modes: None, Random, Rotate, Walk, Scale, Circle, Spiral, Breathe, Wobble, Lissajous, each with mode-specific tuning controls. **D** opens jiggle diagnostics. **bg** sets the canvas background color (24 preset colors). **cfg** opens the solver config popup to switch between JS and WASM engines + set jiggle interval. See [Paths](docs/paths.md) for jiggle formulas. A zero-padded elapsed seconds counter appears during computation. |
