...
- **Delete old parser internals only *after* equivalence passes:** the legacy coeff-only parsing helpers are removed in the **same change** that flips coeff to the core — never before the gate is green, so there's no window with two diverging coeff parsers in production.
...
- **Equivalence asserted per corpus item:** old coeff parser vs shared-core coeff profile must agree on (a) the **compiled chain**, (b) the **fingerprint**, (c) the **diagnostics shape** (line/col + message-class, both strict and non-strict), and (d) **canonical source regeneration** (`coeff_source_text_from_chain` round-trips to an equal `execution_spec`/fingerprint).
