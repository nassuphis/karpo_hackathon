...
**Forward (not a defect):** the M3 wire-packer rewrite itself has not been attempted. It is now startable because the frozen chain oracle will catch wire/fingerprint divergence, but the checked-in corpus is still representative rather than production-wide. Before shipping a real M3 rewrite, expand the corpus from saved `calc.json` payloads and keep the scope to chain/wire packing unless the Coeff source semantic lowerers are frozen too.
