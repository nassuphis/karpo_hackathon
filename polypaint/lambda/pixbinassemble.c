/*
 * pixbinassemble: assemble sparse pixel-bin entries into a dense uint8 tile.
 *
 * Reads packed 8-byte entries from stdin:
...
int main(int argc, char **argv) {
    ...
    if (tileW <= 0 || tileH <= 0 || !outPath) {
        fprintf(stderr,
                "Usage: pixbinassemble --tile_w=TW --tile_h=TH --output=tile.bin [--empty=255]\n");
        return 1;
    }
    ...
    printf("{\"tile_w\":%d,\"tile_h\":%d,\"entries\":%ld,\"pixels\":%zu,\"empty\":%d}\n",
           tileW, tileH, entriesRead, npix, emptyValue);
    return 0;
}
