...
for BIN in "$ROOT/lambda/sweep" "$ROOT/lambda/sweep_mt" "$ROOT/lambda/sweep_cm" "$ROOT/lambda/sweep_coeffgen" "$ROOT/lambda/roots2pix_mt" "$ROOT/lambda/solve_palette_chunk_mt" "$ROOT/lambda/solve_proximity_hist_sectioned" "$ROOT/lambda/bilevel_section_raster" "$ROOT/lambda/coeffs_bilevel_raster" "$ROOT/lambda/bilevel_merge" "$ROOT/lambda/raw_to_bilevel" "$ROOT/lambda/assemble_greyscale"; do
    if [ ! -f "$BIN" ]; then
