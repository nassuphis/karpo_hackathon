...
**Fixes commits (re-audited):** `3760114` "Harden param coeff help contracts" then `1a0fa61` "Finish param coeff help cleanup" — together resolve the original bugs (BUG-1..6) and careless assumptions (CA-1..5). **Subsequent follow-ups through `f94a2d8`:** the Rejected-Forms lookup fragility is fixed, the pre-M3 chain-oracle/calc-corpus gate exists, the forward Coeff M3 packer rewrite landed behind that gate, and the harvested production corpus is checked in. Per-item status in **Resolution Update** and **M3 Packer Rewrite** below.
...
### M3 Packer Rewrite — landed and independently verified (`79895c0`, `55fbe3d`)

The highest-risk piece of the whole plan — the Coeff M3 wire-packer rewrite — has now landed *behind* the oracle gate, and I re-verified it directly (the compiled wire is a permanent fingerprint / render-artifact cache key; a mistake here silently orphans cached renders).

What landed (`79895c0` "Add coeff M3 signature packer"):
- **All five old per-function packers are gone** (`_affine_pair_legacy_args`/`_linear_legacy_args`/`_pow_legacy_args`/`_exp_legacy_args`/`_round_legacy_args` → 0 occurrences), replaced by one **registry-signature-driven interpreter** (`_compat_signature_args`/`_pack_compat_signature`, `coeff_program_chain.py:1316-1376`) driven by `compat_signatures` declared in `lambda/coeff_legacy_registry.json` for exactly **`linear`/`exp`/`round`/`pow`** — the four heterogeneous packers the plan flagged. `_token`/`_execution_spec`/`_fingerprint`/`_legacy_transforms` (the inverse) are intact, as the plan predicted the inverse would *not* collapse. Net `coeff_program_chain.py` +140/−123. Structurally pinned by `test_coeff_program_drift.py:383` (compat_signature transforms) and `:402` (forward packer uses the interpreter, not fn-specific helpers).

Wire-preservation — **proven three ways, all green**:
1. **Frozen oracle, confirmed untouched.** `git diff --quiet adfa918 HEAD` over every `*_legacy.py` is clean — the frozen packer oracle was **not** edited alongside the rewrite (the one way this gate could be faked). `tests/test_program_m3_oracles.py` compares the rewritten live packer vs the frozen old packer and passes (4 passed, **28 subtests**). The synthetic `persisted_legacy_component_chains.calc.json` case exercises **all four** rewritten functions (verified `linear`/`exp`/`round`/`pow` all present), so the oracle genuinely covers the rewrite, not just the surrounding pipeline.
2. **Golden hex.** `test_coeff_wire_fingerprints.py::test_legacy_chain_packed_forms_keep_golden_fingerprints` (pre-refactor frozen fingerprints, untouched by the rewrite) still passes.
3. The structural drift pins above.

Real-production breadth (`55fbe3d` "Harvest production M3 oracle cases"): `scripts/harvest_program_m3_oracle_corpus.py` snapshotted **6 real saved `calc.json` programs** (including an 806-line one) into `tests/fixtures/program-m3-oracle/harvested/`, folded into the oracle test. **Honest scope note:** these real programs exercise `legacy`/`rev` transforms broadly but do **not** use `linear`/`exp`/`round`/`pow` by name — so wire-coverage of the four rewritten functions rests on the synthetic `persisted_legacy_component_chains` case + golden-hex test, while the harvested cases prove the broader pipeline stays byte-identical on real data.

**Verdict:** the M3 forward-packer rewrite is wire-preserving and correctly gated — it landed without moving a fingerprint, which is exactly what the frozen-oracle gate was built to guarantee. This was the single most dangerous change in the plan, and it was executed safely.

### Codex current audit after `f94a2d8`

I agree with the review's current conclusion, with one boundary made explicit. The Help-file defects are closed, and the M3 forward-packer rewrite has the right proof stack: frozen oracles unchanged since `adfa918`, checked-in synthetic coverage for `linear`/`exp`/`round`/`pow`, golden fingerprint coverage, and six real production calc fixtures. I re-ran the focused oracle gate at `f94a2d8`; it is still green (`4 passed, 28 subtests`), and `git diff --name-only adfa918 HEAD -- lambda/*_legacy.py` is empty, so the old side of the comparison was not accidentally moved with the rewrite.
...
**Forward status after the M3 follow-up:** the first wire-compatible M3 packer rewrite has now landed for the forward Coeff packer. `linear`/`exp`/`round`/`pow` declare `compat_signatures` in `lambda/coeff_legacy_registry.json`, `coeff_program_chain.py` uses one signature interpreter instead of the old per-function forward helpers, and `tests/test_coeff_program_drift.py` pins that architecture. The frozen oracle and golden fingerprint tests are green, so the checked corpus keeps byte-identical `execution_spec`/`fingerprint`. **`55fbe3d` closed the production-coverage boundary:** `scripts/harvest_program_m3_oracle_corpus.py` snapshotted 6 real saved `calc.json` programs into the corpus (oracle test → 28 subtests). The frozen oracles are provably unchanged since creation (`git diff --quiet adfa918 HEAD` on all `*_legacy.py`), so the comparison is genuine. See **M3 Packer Rewrite** above for the full verification — the rewrite is wire-preserving and gated; the only M3 work left is the inverse/serializer, which the plan always expected to stay largely per-function unless broader complex forms are added later.
