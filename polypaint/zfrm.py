...
def funvec(fnms):
    names = [name.strip() for name in fnms.split(',')]
    funs = []
    for name in names:
        funs.append(globals().get(name,lambda cf: cf))
    return funs

def sbs(cf):
    global sbs_zfrm1
    global sbs_zfrm2
    if sbs_zfrm1 is None: 
       zfrm1 = ps.poly.get("zfrm1") or "none"
       sbs_zfrm1 = funvec(zfrm1)
    if sbs_zfrm2 is None: 
       zfrm2 = ps.poly.get("zfrm2") or "none"
       sbs_zfrm2 = funvec(zfrm2)
    sbs= ps.poly.get("sbs_rnd") or "concat"
    ro = ps.poly.get("ro") or 0
    io = ps.poly.get("io") or 0
    a = ro+1j*io
    rts1=solve.polyroot(pf.zfrm(cf,sbs_zfrm1))
    rts2=solve.polyroot(pf.zfrm(cf,sbs_zfrm2))+a   
    if sbs=="rnd":
      ...
      rts = np.concatenate((rts1,rts2))
    cf = np.poly(rts)
    max_real = float(np.max(cf.real))
    max_imag = float(np.max(cf.imag))
    min_real = float(np.min(cf.real))
    min_imag = float(np.min(cf.imag))
    if "max_real" in ps.poly and "max_imag" in ps.poly:
      ps.poly["max_real"]=max(max_real,ps.poly["max_real"])
      ps.poly["max_imag"]=max(max_imag,ps.poly["max_imag"])
      ps.poly["min_real"]=max(min_real,ps.poly["min_real"])
      ps.poly["min_imag"]=max(min_imag,ps.poly["min_imag"])
    else:
      ps.poly["max_real"]=max_real
      ps.poly["max_imag"]=max_imag
      ps.poly["min_real"]=min_real
      ps.poly["min_imag"]=min_imag
    return cf
