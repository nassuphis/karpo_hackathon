...
Reviewed at HEAD `1a1996d`. Five parallel read-only sweeps (coeff VM, param VM, solve-score, root transforms, JS editor + fingerprint blast-radius); every load-bearing claim re-verified against source by me, file:line throughout.
...
3. **Text-first is per-DSL, not uniform** (see §9.3): coeff already has a text parser; **param and root** need new ones (param is a coeff-parser subset, root is a flat list); **solve-score stays chip-primary** — its declarative slot-CSE + cross-step lag has no clean text form — with text as a read-only export.
...
## Current state — what is implemented today (verified at HEAD `1a1996d`)
...
**DD2 — Native version gate (Phase 0; the first hard prerequisite).** Add a `version` field to the token-stream wire and make the native parser (`sweep_cli.c:3741,6267`) **reject unknown/newer versions** with a clear error. "No behavior change *for valid v1 payloads*"; intentionally a change for *unsupported* payloads. Converts the worst failure — silent misexecution of a renumbered v2 stream by rolled-back v1 (§7.4) — into a clean reject. Must land **before any opcode renumbering**. **Rollback-floor rule:** the gate only protects a rollback whose target *also* has it — a pre-DD2 binary ignores the `version` field and would still misexecute a v2 stream. So once DD2 ships, **pre-DD2 native binaries are no longer valid rollback targets** (the first DD2 build is the rollback floor); deploy/runbook must record this.
...
One `text → chips` parser **for the text-first profiles** (coeff/param/root — per-use-case grammar sharing one lexer); **solve-score stays chip-chain primary** (chips → tokens directly, with read-only text export, §9.3). One `chips → tokens` compiler (config-parametrized `_compile_chip` + `_validate_stack`), one `execution_spec` renderer, one `sha1`/`sha256` fingerprint. Macros carry over unchanged. The chip-list stays as the intermediate IR (it's the natural migration seam and keeps macros/legacy round-tripping working).
...
- **Two text parsers must be built** (param, root) for text-first authoring — coeff already has one (`coeff_program_source.py`). **Solve-score stays chip-primary** with a read-only text export (§9.3): its declarative metric-prepass + slot-CSE + cross-step lag has no clean text shape. Root is trivial (a flat one-transform-per-line list); param is a subset of coeff's parser — neither is a coeff-sized chunk.
...
- **Phase 3 — build the two new text parsers** (param, root) with `source_text` round-trips; **solve-score stays chip-primary** (text export only, §9.3). *Low-to-medium effort (root is trivial, param is lexer-reuse), low risk (additive).*
...
The refactor has a real UI dimension (D5 alternative 4: "make text the only source and render chips as a read-only visualization") plus the new Legacy/migrate tab. This is its own workstream, gated behind the two new text parsers (param, root; Phase 3) — solve-score stays chip-primary.
...
3. **Greenfield error UX.** The two new parsers (param, root) need diagnostics on par with coeff's line/column (CR15 set the bar); chips currently surface errors inline, text must match.
